...
export async function getWhatsAppReceptionistBusinessContext(userId: string) {
  const [profileRes, servicesRes, staffRes, connectionRes] = await Promise.all([
    ...
    supabaseAdmin
      ...
      .select("business_name,country_code,default_language,display_phone_number,status,ai_receptionist_enabled,ai_receptionist_status,ai_receptionist_handoff_phone,ai_receptionist_notes")
      .eq("user_id", userId)
      .maybeSingle(),
  ]);

...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const verifyAccess = async () => {
      ...
      // În Demo permitem DOAR programările
      if (isDemo && !session && isTryingToAccessStrictAdmin && !path.startsWith("/programari")) {
        router.push("/login");
        return;
      }
      ...
      <body className="antialiased bg-slate-50 min-h-screen font-sans flex flex-col text-slate-900">
        ...
        {isDemoMode && !isLoggedIn && (
          <div className="bg-slate-900 text-white text-[10px] font-black py-2.5 px-4 text-center uppercase tracking-[0.2em] sticky top-0 z-[110] border-b border-amber-500/30">
            ...
            <Link href="/login?view=sign_up" className="text-amber-400 hover:text-white transition-colors underline underline-offset-4">
            ...
            {(isLoggedIn || isDemoMode) && !isStaticCustomerPage && (
              <div className="flex-1 flex justify-center px-2">
                <span className="bg-slate-50 px-4 py-1.5 rounded-full text-[9px] md:text-xs font-black uppercase italic text-amber-600 border border-slate-200 tracking-widest text-center">
                  {isDemoMode && !isLoggedIn ? "Calendar Demo" : currentPage}
                </span>
              </div>
            )}
          ...
          <div className="max-w-[1400px] mx-auto px-6 py-4 flex flex-col md:flex-row justify-between items-center gap-4">
              <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col text-left">
                  ...
                  <a href="mailto:copedatoro@gmail.com" className="text-[12px] font-bold text-amber-600 hover:text-amber-700 underline underline-offset-2 transition-all">
                    copedatoro@gmail.com
                  </a>
                </div>
              </div>
              <div className="flex flex-wrap justify-center gap-x-5 gap-y-2">
                <Link href="/sugestii" className="text-[10px] font-black text-amber-600 uppercase tracking-widest hover:text-amber-700">Sugestii ⭐</Link>
                <Link href="/termeni" className="text-[10px] font-black text-slate-500 uppercase tracking-widest hover:text-amber-600">Termeni</Link>
                <Link href="/confidentialitate" className="text-[10px] font-black text-slate-500 uppercase tracking-widest hover:text-amber-600">Confidențialitate</Link>
                <Link href="/cookies" className="text-[10px] font-black text-slate-500 uppercase tracking-widest hover:text-amber-600">Cookies</Link>
              </div>

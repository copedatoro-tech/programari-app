  ...
  return (
    <main className="min-h-screen bg-slate-50 py-10 px-4 md:px-8 font-sans">
      {showModal && (
        <ElegantModal
          title={t("modal.title")}
          message={t("modal.message", { plan: showModal.name })}
          confirmLabel={t("modal.confirmBtn")}
          cancelLabel={t("modal.cancelBtn")}
      ...
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-8 space-y-2">
          <h1 className="text-3xl md:text-4xl font-black italic tracking-tighter text-slate-900 uppercase">
            {t("header.title")} <span className="text-amber-500">{t("header.titleHighlight")}</span>
          </h1>
            ...
            <div className="flex items-center gap-3">
              {isTrialActive ? (
                <div className="flex gap-1.5">
                  {[
                    { l: t("countdown.days"), v: timeLeft.zile },
                    { l: t("countdown.hours"), v: timeLeft.ore },
                    { l: t("countdown.min"), v: timeLeft.minute },
                    { l: t("countdown.sec"), v: timeLeft.secunde }
                  ].map((tm, i) => (
                    <div key={i} className="bg-slate-900 w-14 h-14 rounded-xl flex flex-col items-center justify-center shadow-md border-b-2 border-slate-700">
                      <span className="text-white font-black text-lg leading-none">{tm.v}</span>
                      <span className="text-[7px] text-slate-400 font-bold uppercase">{tm.l}</span>
                    </div>
                  ))}
                </div>
              ) : (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 items-stretch">
          {plans.map((plan, index) => {
            ...
            const isSelected = currentPlan === meta.id;
            const eurApprox = Math.round(plan.price / RON_TO_EUR);
            return (
              <div
                key={index}

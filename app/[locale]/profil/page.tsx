...
export default function ProfilPage() {
  ...
  const handleConfirmPassword = async () => {
    if (isDemo) return;
    if (!pass1 || pass1 !== pass2) { alert(t("passwordMismatch")); return; }
    setUpdating(true);
    const { error } = await supabase.auth.updateUser({ password: pass1 });
    if (error) alert(error.message);
    else { alert(t("passwordUpdated")); setShowPassModal(false); setPass1(""); setPass2(""); }
    setUpdating(false);
  };
  ...
  if (!isClient || loading) return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50">
      ...
      <p className="italic font-black text-slate-400 animate-pulse uppercase tracking-[0.3em] text-[10px]">{t("loading")}</p>
    </div>
  );
        ...
        <div className="bg-slate-900 p-12 flex flex-col md:flex-row items-center gap-10 relative overflow-hidden">
          ...
          <div className="relative w-40 h-40 shrink-0 group z-10">
            ...
            {!isDemo && (
              <label className="absolute inset-0 flex items-center justify-center bg-slate-900/80 opacity-0 group-hover:opacity-100 transition-all cursor-pointer rounded-[45px] m-1 z-20">
                ...
                <span className="text-[10px] font-black text-amber-500 uppercase italic">{t("changeAvatar")}</span>
              </label>
            )}
          </div>
          ...
          <div className="flex-1 text-center md:text-left z-10">
            <div className="inline-block px-3 py-1 bg-white/5 border border-white/10 rounded-full mb-4 text-amber-500 text-[9px] font-black uppercase tracking-[0.4em] italic">
                {isDemo ? t("demoMode") : `${t("idPrefix")}${user?.id?.substring(0, 8)}`}
            </div>
          ...
          <div className="flex flex-col gap-4 z-10">
            ...
            <button title={t("logout")} onClick={handleSignOut} className="px-10 py-4 bg-slate-800 text-slate-400 text-[11px] font-black rounded-2xl uppercase italic border-b-8 border-slate-700 hover:bg-red-600 hover:text-white transition-all">
              {isDemo ? t("demoLogout") : t("logout")}
            </button>
          </div>
        </div>
            ...
            <div>
              ...
              <h3 className={`text-2xl font-black italic uppercase ${LIMITE_ABONAMENTE[subscriptionPlan.toLowerCase()]?.culoare || 'text-slate-900'}`}>
                ...
                {(isTrialActive || subscriptionPlan.includes("trial")) && (
                  <span className="ml-2 text-[10px] text-amber-600 animate-pulse">{t("trialSuffix")}</span>
                )}
              </h3>
              <p className="text-[10px] font-bold text-slate-500 italic uppercase">
                {LIMITE_ABONAMENTE[subscriptionPlan.toLowerCase()]?.limita || t("plans.free.limita")}
              </p>
            </div>
          </div>
          <button title={t("changeSubscriptionBtn")} onClick={() => router.push('/abonamente')} className="px-8 py-4 bg-slate-900 text-white text-[10px] font-black rounded-2xl uppercase italic hover:bg-amber-500 hover:text-slate-900 border-b-8 border-slate-800 transition-all shadow-lg">
            {t("changeSubscriptionBtn")}
          </button>
        </div>
        ...
        <div className="p-12 space-y-12 bg-white">
          ...
          <div className="pt-6 border-t border-slate-50 flex justify-center">
            <button
              ...
              onClick={() => { if(!isDemo) setShowPassModal(true); else alert(t("demoDisabled")); }}
        ...
        <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-md z-[200] flex items-center justify-center p-6" onClick={() => setShowPassModal(false)}>
          <div className="bg-white w-full max-w-sm rounded-[50px] p-12 shadow-2xl space-y-10 border border-slate-100 relative" onClick={(e) => e.stopPropagation()}>
            <div className="text-center">
              <h2 className="text-3xl font-black text-slate-900 uppercase italic">{t("passwordModalTitle")}</h2>
            </div>
            <div className="space-y-4">
              <input type="password" placeholder={t("newPasswordPlaceholder")} value={pass1} onChange={(e) => setPass1(e.target.value)} className="w-full p-6 bg-slate-50 border-2 border-slate-100 rounded-[25px] font-bold text-center outline-none focus:border-amber-500" />
              <input type="password" placeholder={t("confirmPasswordPlaceholder")} value={pass2} onChange={(e) => setPass2(e.target.value)} className="w-full p-6 bg-slate-50 border-2 border-slate-100 rounded-[25px] font-bold text-center outline-none focus:border-amber-500" />
            </div>
            <button title={t("confirmPasswordPlaceholder")} onClick={handleConfirmPassword} className="w-full py-6 bg-slate-900 text-amber-500 rounded-[25px] font-black text-[12px] uppercase italic border-b-8 border-slate-800 hover:bg-amber-500 hover:text-white transition-all">
              {updating ? "..." : t("changePasswordBtn")}
            </button>
          </div>
        </div>
      )}

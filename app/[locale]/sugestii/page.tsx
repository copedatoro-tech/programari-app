...
export default function PareriClienti() {
  const t = useTranslations("sugestii");
  ...
  const trimiteFeedback = async () => {
    ...
    try {
      ...
      alert(t("sendErrorPrefix") + err.message);
    } finally {
      setSeIncarca(false);
    }
  };
  ...
  const aprobaRapid = async (id: string) => {
    ...
    try {
      ...
      alert(t("approveErrorPrefix") + err.message);
    }
  };
  ...
  const dezaprobaRapid = async (id: string) => {
    ...
    try {
      ...
      alert(t("disapproveErrorPrefix") + err.message);
    }
  };
    ...
    return (
      <main className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white p-10 rounded-[50px] shadow-2xl border border-slate-100 text-center">
          ...
          <h2 className="text-2xl font-black uppercase italic mb-4">{t("premiumTitle")}</h2>
          <p className="text-slate-500 font-bold italic text-sm mb-8 leading-relaxed">
            {t("premiumTextBefore")}<span className="text-amber-600 font-black">{t("premiumHighlight")}</span>{t("premiumTextAfter")}
          </p>
          <Link
            href="/abonamente"
    ...
    <main className="min-h-screen bg-slate-50 p-4 md:p-8 font-sans text-slate-900">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
          {[
            { label: t("statTotal"), value: totalRecenzii, color: "bg-slate-900 text-white" },
            { label: t("statApproved"), value: aprobate, color: "bg-emerald-500 text-white" },
            { label: t("statPending"), value: neaprobate, color: "bg-amber-500 text-black" },
            { label: t("statAvgStars"), value: medieStele, color: "bg-white text-slate-900 border-2 border-slate-100" },
          ].map((s) => (
            <div
              key={s.label}
        ...
        <div className="max-w-xl mx-auto mb-16 bg-white p-10 rounded-[50px] shadow-2xl relative border border-slate-100">
          {trimis && (
            <div className="absolute inset-0 bg-white/95 z-50 flex flex-col items-center justify-center text-center p-8 rounded-[50px]">
              <div ref={successPopupRef}>
                ...
                <h2 className="text-2xl font-black uppercase italic">{t("successTitle")}</h2>
                <button
          ...
          <div className="space-y-5">
            ...
            >
              {seIncarca ? t("submitting") : t("submitBtn")}
            </button>
          </div>
        </div>
            ...
            feedbacks.map((f) => (
              ...
              >
                ...
                <div className="absolute top-6 right-6 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-all z-20">
                  {!f.aprobat ? (
                    <button
                      ...
                      title={t("approveBtn")}
                      className="bg-emerald-500 text-white p-3 rounded-2xl text-[9px] font-black uppercase shadow-lg hover:scale-105 transition-transform"
                    >
                      {t("approveBtn")}
                    </button>
                  ) : (
                    <button
                      ...
                      title={t("disapproveBtn")}
                      className="bg-slate-400 text-white p-3 rounded-2xl text-[9px] font-black uppercase shadow-lg hover:scale-105 transition-transform"
                    >
                      {t("disapproveBtn")}
                    </button>
                  )}
                  <button
                    ...
                    title={t("editBtn")}
                    className="bg-slate-900 text-white p-3 rounded-2xl text-[9px] font-black uppercase shadow-lg hover:scale-105 transition-transform"
                  >
                    {t("editBtn")}
                  </button>
                  <button
                    ...
                    title={t("deleteBtn")}
                    className="bg-red-500 text-white p-3 rounded-2xl text-[9px] font-black uppercase shadow-lg hover:scale-105 transition-transform"
                  >
                    {t("deleteBtn")}
                  </button>
                </div>
                <div className="flex gap-1 mb-6 flex-wrap">
                  ...
                  >
                    {f.aprobat ? t("approvedBadge") : t("newBadge")}
                  </span>
                </div>
                {editId === f.id ? (
                  <div className="space-y-4 bg-slate-50 p-6 rounded-3xl border-4 border-amber-500">
                    <label className="text-[9px] font-black uppercase opacity-50">{t("commentLabel")}</label>
                    <textarea
                      className="w-full p-4 text-xs font-bold rounded-xl border-2 outline-none"
                      value={textEditat}
                      onChange={(e) => setTextEditat(e.target.value)}
                    />
                    <label className="text-[9px] font-black uppercase opacity-50">{t("yourResponseLabel")}</label>
                    <textarea
                      className="w-full p-4 text-xs font-bold rounded-xl border-2 border-amber-200 bg-amber-50 outline-none"
                      value={raspunsAdmin}
                ...
                <div className="flex items-center gap-4 border-t border-slate-100 pt-6 mt-auto">
                  ...
                  <div className="flex flex-col">
                    <span className="font-black uppercase text-[11px] italic text-slate-900">{f.nume_client}</span>
                    <span className="text-[8px] font-black text-slate-300 uppercase tracking-widest">
                      {f.created_at ? new Date(f.created_at).toLocaleDateString("ro-RO") : t("recentDateFallback")}
                    </span>
                  </div>
                </div>
              </div>

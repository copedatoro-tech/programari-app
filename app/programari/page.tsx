...
function ProgramariContent() {
  ...
  const serviciiFiltrate = useMemo(() => {
    ...
    return (servicii || []).filter((s: ServiceRow) => specialist.services.includes(s.id));
  ...
  const selecteazaClient = (client: any) => {
    setFormular((prev) => ({ ...prev, nume: client.nume, email: client.email || "", telefon: client.phone || client.telefon || "", poza: client.file_url || null }));
    setShowSuggestions(false);
  };
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of Array.from(e.target.files)) {
      ...
      const { error: uploadError } = await supabase.storage.from("documente-programari").upload(fileName, file);
      if (!uploadError) {
        const { data: publicUrlData } = supabase.storage.from("documente-programari").getPublicUrl(fileName);
        newDocuments.push({ id: Date.now() + Math.random(), name: file.name, url: publicUrlData.publicUrl });
      }
    }
    ...
    <main className="min-h-screen bg-slate-50 p-4 md:p-12 text-slate-900 font-sans">
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-4">
          <div>
            <h1 className="text-4xl md:text-5xl font-black italic uppercase tracking-tighter leading-none">Gestiune <span className="text-amber-600">Programări</span></h1>
            <p className="text-[10px] font-black uppercase italic text-slate-400 mt-2">Plan: <span className="text-amber-600">{userPlan.toUpperCase()}</span> • {countLunaCurenta} / {isTrialing ? "∞" : limitaCurenta} luna aceasta</p>
          </div>
      ...
      {popupProgramare && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-[200] flex items-center justify-center p-4" onClick={() => setPopupProgramare(null)}>
          <div ref={popupRef} className="bg-white w-full max-w-lg rounded-[50px] overflow-hidden shadow-2xl border border-slate-100 relative" onClick={(e) => e.stopPropagation()}>
            <button
            ...
            <div className="pt-16 p-10">
              ...
              <div className="grid grid-cols-2 gap-4 mb-8">
                ...
                <div className="bg-slate-50 p-4 rounded-3xl border border-slate-100">
                  ...
                  <p className="font-black text-xs text-slate-700">{angajati?.find((a: any) => a.id === popupProgramare.angajat_id)?.name || "General"}</p>
                </div>
                <div className="bg-slate-50 p-4 rounded-3xl border border-slate-100">

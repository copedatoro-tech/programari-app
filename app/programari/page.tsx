...
function ProgramariContent() {
    ...
    if (profileData) {
      ...
      if (profileData.trial_started_at) {
        ...
        if (acum - start < zeceZileInMs) {
          ...
          setIsTrialing(true); setDaysLeft(dLeft); plan = "chronos team";
  ...
  const serviciiFiltrate = useMemo(() => {
    ...
    if (!specialist || !specialist.services || specialist.services.length === 0) return servicii;
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (e: MouseEvent) => {
      ...
      if (pickerRef.current && !pickerRef.current.contains(target)) { setShowPicker(false); setPickerStep("hours"); }
      if (popupRef.current && !popupRef.current.contains(target)) setPopupProgramare(null);
    };
  ...
  return (
    <main className="min-h-screen bg-slate-50 p-4 md:p-12 text-slate-900 font-sans">
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-4">
          ...
          <div className="flex flex-col gap-2 self-start md:self-auto items-end">
            ...
            <div className="flex gap-2">
              <Link href="/programari/calendar" title="Deschide calendarul complet" className="bg-white px-6 py-3 rounded-2xl shadow-sm border border-slate-200 flex items-center gap-3 hover:bg-slate-50 transition-all active:scale-95 group">
        ...
        <section className="bg-white rounded-[50px] p-8 md:p-14 shadow-2xl border border-slate-100 mb-16 relative">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
            ...
            <div className="lg:col-span-9 grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="flex flex-col gap-2"><label className="text-[10px] font-black uppercase ml-4 text-slate-400 italic">E-mail</label><input type="email" placeholder="client@email.com" className="p-5 bg-slate-50 rounded-[25px] border-2 border-transparent focus:border-amber-500 font-bold text-lg outline-none shadow-inner" value={formular.email} onChange={e => setFormular({ ...formular, email: e.target.value })} /></div>
              <div className="flex flex-col gap-2"><label className="text-[10px] font-black uppercase ml-4 text-slate-400 italic">Telefon</label><input type="tel" placeholder="07xxxxxxxx" className="p-5 bg-slate-50 rounded-[25px] border-2 border-transparent focus:border-amber-500 font-bold text-lg outline-none shadow-inner" value={formular.telefon} onChange={e => setFormular({ ...formular, telefon: e.target.value })} /></div>
              <div className="flex flex-col gap-2"><label className="text-[10px] font-black uppercase ml-4 text-slate-400 italic">Alege Specialist {formular.serviciu_id && <span className="text-amber-500 ml-1">(filtrat)</span>}</label><select className="p-5 bg-slate-50 rounded-[25px] border-2 border-transparent focus:border-amber-500 font-bold text-lg outline-none shadow-inner cursor-pointer" value={formular.angajat_id} onChange={e => setFormular({ ...formular, angajat_id: e.target.value })}><option value="">Alege Specialist...</option>{angajatiFiltrati.map(a => <option key={a.id} value={a.id}>{a.name}</option>)}</select></div>
              <div className="flex flex-col gap-2"><label className="text-[10px] font-black uppercase ml-4 text-slate-400 italic">Alege Serviciu {formular.angajat_id && <span className="text-amber-500 ml-1">(filtrat)</span>}</label><select className="p-5 bg-slate-50 rounded-[25px] border-2 border-transparent focus:border-amber-500 font-bold text-lg outline-none shadow-inner cursor-pointer" value={formular.serviciu_id} onChange={e => setFormular({ ...formular, serviciu_id: e.target.value })}><option value="">Alege Serviciu...</option>{serviciiFiltrate.map(s => <option key={s.id} value={s.id}>{s.nume_serviciu} — {s.price} RON</option>)}</select></div>
              <div className="flex flex-col gap-2"><label className="text-[10px] font-black uppercase ml-4 text-slate-400 italic">Data</label><input type="date" className="p-5 bg-slate-50 rounded-[25px] border-2 border-transparent focus:border-amber-500 font-bold text-lg outline-none shadow-inner" value={formular.data} onChange={e => setFormular({ ...formular, data: e.target.value })} /></div>
              <div className="flex flex-col gap-2"><label className="text-[10px] font-black uppercase ml-4 text-slate-400 italic">Ora</label><button type="button" title="Alege ora programării" onClick={() => setShowPicker(true)} className="w-full p-5 bg-slate-50 rounded-[25px] font-bold text-lg shadow-inner text-left flex justify-between items-center border-2 border-transparent hover:border-amber-500 transition-all active:scale-95">{formular.ora} <span className="text-amber-600 text-[10px]">🕒</span></button></div>
              <div className="md:col-span-2 flex flex-col gap-2"><label className="text-[10px] font-black uppercase ml-4 text-slate-400 italic">Observații</label><textarea placeholder="De ce vine clientul?" className="p-5 bg-slate-50 rounded-[25px] border-2 border-transparent focus:border-amber-500 font-bold text-lg h-16 resize-none outline-none shadow-inner" value={formular.motiv} onChange={e => setFormular({ ...formular, motiv: e.target.value })} /></div>
            </div>
          </div>
          <div className="mt-8 pt-8 border-t border-slate-100 flex flex-col lg:flex-row gap-4 items-center">
            <div className="flex-1 w-full bg-slate-100/50 p-4 rounded-[30px] border border-slate-200">
              <div className="flex items-center justify-between mb-3 px-2"><span className="text-[9px] font-black uppercase text-slate-500 italic">Fișiere atașate</span><input type="file" id="doc-upload" className="hidden" multiple onChange={handleFileUpload} /><label htmlFor="doc-upload" title="Adaugă documente sau imagini" className="bg-slate-900 text-white px-5 py-2.5 rounded-xl font-black text-[10px] uppercase italic cursor-pointer hover:bg-amber-600 transition-colors shadow-sm active:scale-95">Adaugă Fișier +</label></div>
              <div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto p-1">
                {formular.documente.map(doc => (
                  <div key={doc.id} className="relative flex items-center gap-2 w-auto max-w-[180px] h-10 pr-8 pl-2 bg-white border border-slate-200 rounded-xl shadow-sm">
                    <div className="w-6 h-6 rounded-md bg-slate-50 flex-shrink-0 overflow-hidden">{doc.url.startsWith("data:image") ? <img src={doc.url} className="w-full h-full object-cover" /> : <span className="flex items-center justify-center h-full text-[10px]">📄</span>}</div>
                    <span className="text-[8px] font-black text-slate-600 truncate uppercase italic">{doc.name}</span>
                    <button onClick={() => eliminaDocument(doc.id)} title="Șterge fișier" className="absolute right-1.5 w-5 h-5 bg-red-50 text-red-500 rounded-lg flex items-center justify-center text-[10px] font-black hover:bg-red-500 hover:text-white transition-all active:scale-90">✕</button>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 pb-40">
          {programariAzi.length === 0 ? (
            <div className="col-span-full py-12 text-center bg-white rounded-[35px] border-2 border-dashed border-slate-100"><p className="text-[10px] font-black uppercase italic text-slate-300">Nicio programare pentru azi.</p></div>
          ) : (
            programariAzi.map(p => (
              <div key={p.id} title="Click pentru detalii" className="relative bg-white p-5 rounded-[35px] shadow-sm border border-amber-200 ring-2 ring-amber-100 transition-all cursor-pointer hover:shadow-lg hover:scale-[1.02] active:scale-95" onClick={() => setPopupProgramare(p)}>
                <button onClick={e => eliminaProgramare(p.id, e)} title="Șterge programarea" className="absolute top-4 right-4 text-red-500 font-black text-[10px] z-10 hover:scale-125 transition-transform active:scale-90">✕</button>
                <div className="flex gap-3 items-center mb-4 pr-6">
                  <div className="w-12 h-12 rounded-[18px] bg-slate-50 overflow-hidden border-2 border-white shadow-inner flex items-center justify-center relative">
                    {p.poza ? <img src={p.poza} className="w-full h-full object-cover" /> : <Image src="/logo-chronos.png" alt="logo" fill sizes="48px" style={{ objectFit: 'contain', padding: '4px' }} />}
                  </div>
                  <div className="overflow-hidden flex-1">
                    <h4 className="font-black text-slate-800 uppercase text-[11px] truncate italic leading-tight">{p.nume}</h4>
                    <p className="text-[9px] font-black text-amber-600 uppercase italic">{p.ora} • {angajati.find(a => a.id === p.angajat_id)?.name || 'General'}</p>
                    <p className="text-[9px] font-bold text-slate-400 italic uppercase">{servicii.find(s => s.id === p.serviciu_id)?.nume_serviciu || 'Procedură'}</p>
                  </div>
                </div>
                <div className="bg-slate-50 p-2 rounded-xl"><p className="text-[8px] font-black text-slate-400 uppercase italic truncate">{p.motiv || "Fără detalii"}</p></div>
              </div>
            ))
          )}
        </div>
      ...
      {showPicker && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-center justify-center p-6" onClick={() => { setShowPicker(false); setPickerStep("hours"); }}>
          <div ref={pickerRef} className="bg-white w-full max-w-[420px] rounded-[45px] p-8 relative shadow-2xl border border-slate-100 flex flex-col animate-in fade-in zoom-in duration-200" onClick={e => e.stopPropagation()}>
            <button onClick={() => { setShowPicker(false); setPickerStep("hours"); }} className="absolute top-6 right-6 text-slate-300 hover:text-red-500 text-xl font-black transition-colors" title="Închide">✕</button>
            <div className="flex flex-col gap-1 mb-8"><p className="text-[9px] font-black text-amber-500 tracking-[0.3em] uppercase">{pickerStep === "hours" ? "Pasul 1: Alege Ora" : "Pasul 2: Alege Minutele"}</p><h2 className="font-black uppercase italic text-2xl text-slate-900 tracking-tighter">Selectează Timpul</h2></div>
            <div className="bg-slate-900 mb-6 p-5 rounded-[30px] flex items-center justify-center gap-4 border-b-4 border-amber-500 shadow-inner">
              <span className={`text-4xl font-black ${pickerStep === "hours" ? "text-amber-500 animate-pulse" : "text-white"}`}>{selectedHour.toString().padStart(2, "0")}</span><span className="text-amber-500 font-black text-3xl">:</span><span className={`text-4xl font-black ${pickerStep === "minutes" ? "text-amber-500 animate-pulse" : "text-white"}`}>{selectedMinute.toString().padStart(2, "0")}</span>
            </div>
            <div className="flex-1 bg-slate-50 p-6 rounded-[35px] border border-slate-100">
              {pickerStep === "hours" ? (
                <div className="grid grid-cols-4 gap-3 max-h-[300px] overflow-y-auto pr-2 scrollbar-hide">
                  {Array.from({ length: 24 }).map((_, i) => (<button key={i} onClick={() => { setSelectedHour(i); setPickerStep("minutes"); }} className={`aspect-square rounded-[20px] font-black text-base flex items-center justify-center transition-all ${selectedHour === i ? "bg-amber-500 text-slate-900 shadow-lg scale-105" : "text-slate-900 bg-white hover:border-amber-500 border-2 border-transparent active:scale-95"}`}>{i.toString().padStart(2, "0")}</button>))}
                </div>
              ) : (
                <div className="flex flex-col gap-3">
                  {[0, 15, 30, 45].map(m => (<button key={m} onClick={() => { setSelectedMinute(m); setShowPicker(false); setPickerStep("hours"); }} className={`w-full py-6 rounded-[25px] font-black text-2xl transition-all border-2 flex items-center justify-center gap-3 ${selectedMinute === m ? "bg-slate-900 text-amber-500 border-amber-500 shadow-xl" : "bg-white text-slate-400 border-slate-100 hover:text-slate-900 active:scale-95"}`}><span className="text-[10px] opacity-40 uppercase font-black">Minutul</span>{m.toString().padStart(2, "0")}</button>))}
                  <button onClick={() => setPickerStep("hours")} className="mt-2 py-2 text-slate-400 font-black uppercase italic text-[10px] hover:text-amber-600 transition-colors">← Înapoi la ore</button>
                </div>
              )}
            </div>
          </div>
      ...
      {popupProgramare && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-[200] flex items-center justify-center p-4" onClick={() => setPopupProgramare(null)}>
          <div ref={popupRef} className="bg-white w-full max-w-lg rounded-[50px] overflow-hidden shadow-2xl border border-slate-100 relative animate-in fade-in zoom-in duration-200" onClick={e => e.stopPropagation()}>
            <button onClick={() => setPopupProgramare(null)} title="Închide detaliile" className="absolute top-8 right-8 w-10 h-10 bg-slate-50 rounded-2xl flex items-center justify-center font-black text-slate-400 hover:bg-red-50 hover:text-red-500 transition-all z-10 active:scale-90">✕</button>
            <div className="h-32 bg-slate-900 relative"><div className="absolute -bottom-12 left-10 w-24 h-24 rounded-[30px] bg-white p-2 shadow-xl border border-slate-50"><div className="w-full h-full rounded-[22px] bg-slate-50 overflow-hidden relative flex items-center justify-center">{popupProgramare.poza ? <img src={popupProgramare.poza} className="w-full h-full object-cover" /> : <Image src="/logo-chronos.png" alt="logo" fill sizes="80px" style={{ objectFit: 'contain', padding: '8px' }} />}</div></div></div>
            <div className="pt-16 p-10">
              <div className="mb-6"><h3 className="text-2xl font-black uppercase italic tracking-tighter text-slate-900 leading-none">{popupProgramare.nume}</h3><p className="text-amber-600 font-black text-[10px] uppercase italic mt-1 tracking-widest">{popupProgramare.data} la ora {popupProgramare.ora}</p></div>
              <div className="grid grid-cols-2 gap-4 mb-8">
                ...
                <div className="bg-slate-50 p-4 rounded-3xl border border-slate-100"><p className="text-[8px] font-black text-slate-400 uppercase italic mb-1">Specialist</p><p className="font-black text-xs text-slate-700">{angajati.find(a => a.id === popupProgramare.angajat_id)?.name || 'General'}</p></div>
                <div className="bg-slate-50 p-4 rounded-3xl border border-slate-100"><p className="text-[8px] font-black text-slate-400 uppercase italic mb-1">Serviciu</p><p className="font-black text-xs text-slate-700">{servicii.find(s => s.id === popupProgramare.serviciu_id)?.nume_serviciu || 'Procedură'}</p></div>
              </div>
              <div className="bg-slate-900 p-6 rounded-[35px] text-white"><p className="text-[8px] font-black text-amber-500 uppercase italic mb-2">Motivul vizitei</p><p className="text-xs font-medium italic opacity-90">{popupProgramare.motiv || "Fără observații."}</p></div>
            </div>
          </div>
        </div>
      )}

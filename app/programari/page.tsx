...
// ✅ Tipuri aliniate cu tabelele services și staff din Supabase
type StaffRow = { id: string; name: string; services: string[]; };
type ServiceRow = { id: string; name: string; price: number; duration: number; };

type Programare = {
  id: any; nume: string; email: string; data: string; ora: string; motiv: string;
  telefon: string; poza: string | null; reminderMinutes: number; reminderSound: boolean;
  reminderVibration: boolean; reminderVolume: number; sendToClient: boolean;
  documente: DocumentAttachment[]; angajat_id: string; serviciu_id: string; created_by_client?: boolean;
};

const LIMITE_ABONAMENTE: Record<string, number> = {
  "start (gratuit)": 50, "chronos pro": 200, "chronos elite": 1000, "chronos team": 999999
};
...
function ProgramariContent() {
  ...
  // ✅ Stare din tabelele services și staff
  ...
  const [formular, setFormular] = useState<Programare>({
    id: 0, nume: "", email: "", data: new Date().toISOString().split('T')[0], ora: "09:00",
    motiv: "", telefon: "", poza: null, reminderMinutes: 10, reminderSound: true,
    reminderVibration: true, reminderVolume: 70, sendToClient: true, documente: [],
  ...
  // ✅ Filtrare bidirecțională folosind id-urile din tabelele reale
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event) => {
      if (event === "SIGNED_OUT") {
        setProgramari([]); setAngajati([]); setServicii([]); setCountLunaCurenta(0);
      }
    });
  ...
  async function fetchInitialData() {
    ...
    try {
      ...
      await Promise.all([
        ...
        fetchResurse(user.id),       // ✅ Fetch din tabelele dedicate
        checkSubscriptionLimit(user.id)
      ]);
    } catch (err) { console.error(err); } finally { setLoadingDB(false); }
  }

  // ✅ Fetch resurse din tabelele services și staff (nu din profiles)
  async function fetchResurse(userId: string) {
    if (!supabase) return;

    const { data: profileData } = await supabase
      .from('profiles').select('plan_type').eq('id', userId).single();
    if (profileData) setUserPlan(profileData.plan_type?.toLowerCase() || "start (gratuit)");
  ...
  async function checkSubscriptionLimit(userId: string) {
    ...
    const { count } = await supabase.from('appointments').select('*', { count: 'exact', head: true })
      .eq('user_id', userId).gte('date', inceputLuna.toISOString().split('T')[0]);
  ...
  async function fetchProgramari(userId: string) {
    ...
    if (data) {
      setProgramari(data.map((item: any) => ({
        id: item.id, nume: item.title || "", email: item.email || "",
        data: item.date || "", ora: item.time || "", motiv: item.details || "",
        telefon: item.phone || "", poza: item.file_url || null,
        ...
        // ✅ Salvăm id-urile pentru a le găsi în tabelele services/staff
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      Array.from(e.target.files).forEach(file => {
        ...
        reader.onload = () => {
          setFormular(prev => ({ ...prev, documente: [...prev.documente, { id: Date.now() + Math.random(), name: file.name, url: reader.result as string }] }));
        };
  ...
  const salveazaInCloud = async () => {
    ...
    const payload = {
      user_id: user.id, title: formular.nume, email: formular.email,
      date: formular.data, time: formular.ora, details: formular.motiv,
      phone: formular.telefon, file_url: formular.poza, is_client_booking: false,
      // ✅ Salvăm angajat_id și serviciu_id (UUID din tabelele reale)
      notifications: {
        sound: formular.reminderSound, vibration: formular.reminderVibration,
        sendToClient: formular.sendToClient, docs: formular.documente,
        minutes: formular.reminderMinutes, volume: formular.reminderVolume,
        angajat_id: formular.angajat_id, serviciu_id: formular.serviciu_id
      }
    };
      ...
      <div className="max-w-6xl mx-auto">
        {isTrialing && (
          <div className="mb-10 bg-slate-900 border-l-[10px] border-amber-500 p-6 rounded-[35px] shadow-xl flex flex-col md:flex-row items-center justify-between overflow-hidden relative border border-white/5">
            ...
            <div className="mt-4 md:mt-0 px-6 py-2 bg-white/5 rounded-full border border-white/10 relative z-10">
              <p className="text-[10px] font-black text-slate-300 uppercase italic"><span className="text-white text-lg mr-2">{daysLeft}</span> zile rămase</p>
            </div>
          </div>
        )}
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-4">
          <div>
            ...
            <p className="text-[10px] font-black uppercase italic text-slate-400 mt-2">
              Plan: <span className="text-amber-600">{userPlan.toUpperCase()}</span> • {countLunaCurenta} / {limitaCurenta} luna aceasta
            </p>
          </div>
          <div className="flex flex-col gap-2 self-start md:self-auto items-end">
            ...
            <div className="flex gap-2">
              <Link href="/programari/calendar" className="bg-white px-6 py-3 rounded-2xl shadow-sm border border-slate-200 flex items-center gap-3 hover:bg-slate-50 transition-all active:scale-95 group">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
            ...
            <div className="lg:col-span-9 grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="md:col-span-2 flex flex-col gap-2 relative" ref={suggestionsRef}>
                ...
                <input type="text" placeholder="Nume..." className="p-5 bg-slate-50 rounded-[25px] border-2 border-transparent focus:border-amber-500 font-bold text-lg outline-none shadow-inner"
                  value={formular.nume} onChange={e => handleNumeChange(e.target.value)} />
                {showSuggestions && (
              ...
              <div className="flex flex-col gap-2">
                ...
                <input type="email" placeholder="client@email.com" className="p-5 bg-slate-50 rounded-[25px] border-2 border-transparent focus:border-amber-500 font-bold text-lg outline-none shadow-inner"
                  value={formular.email} onChange={e => setFormular({ ...formular, email: e.target.value })} />
              </div>
              <div className="flex flex-col gap-2">
                <label className="text-[10px] font-black uppercase ml-4 text-slate-400 italic">Telefon</label>
                <input type="tel" placeholder="07xxxxxxxx" className="p-5 bg-slate-50 rounded-[25px] border-2 border-transparent focus:border-amber-500 font-bold text-lg outline-none shadow-inner"
                  value={formular.telefon} onChange={e => setFormular({ ...formular, telefon: e.target.value })} />
              </div>

              {/* ✅ Select specialist din tabela staff */}
              <div className="flex flex-col gap-2">
                <label className="text-[10px] font-black uppercase ml-4 text-slate-400 italic">Alege Specialist</label>
                <select className="p-5 bg-slate-50 rounded-[25px] border-2 border-transparent focus:border-amber-500 font-bold text-lg outline-none shadow-inner cursor-pointer"
              ...
              {/* ✅ Select serviciu din tabela services */}
              <div className="flex flex-col gap-2">
              ...
              <div className="flex flex-col gap-2">
                ...
                <input type="date" className="p-5 bg-slate-50 rounded-[25px] border-2 border-transparent focus:border-amber-500 font-bold text-lg outline-none shadow-inner"
                  value={formular.data} onChange={e => setFormular({ ...formular, data: e.target.value })} />
              </div>
              ...
              <div className="flex flex-col gap-2 relative" ref={hourPickerRef}>
                ...
                <button type="button" onClick={() => setShowHourPicker(!showHourPicker)}
                    ...
                    <div className="flex gap-4 h-44">
                      <div className="flex-1 overflow-y-auto text-center">
                        {Array.from({ length: 24 }, (_, i) => i.toString().padStart(2, '0')).map(h => (
                          <button key={h} onClick={() => setTempHour(h)}
                            className={`w-full py-2 rounded-xl font-black transition-colors ${tempHour === h ? 'bg-amber-600 text-white' : 'text-slate-400 hover:bg-slate-50'}`}>{h}</button>
                        ))}
                      </div>
                      <div className="flex-1 overflow-y-auto text-center">
                        {["00", "15", "30", "45"].map(m => (
                          <button key={m} onClick={() => setTempMin(m)}
                            className={`w-full py-2 rounded-xl font-black transition-colors ${tempMin === m ? 'bg-slate-800 text-white' : 'text-slate-400 hover:bg-slate-50'}`}>{m}</button>
                        ))}
                      </div>
                    </div>
              ...
              <div className="md:col-span-2 flex flex-col gap-2">
                ...
                <textarea placeholder="De ce vine clientul?" className="p-5 bg-slate-50 rounded-[25px] border-2 border-transparent focus:border-amber-500 font-bold text-lg h-16 resize-none outline-none shadow-inner"
                  value={formular.motiv} onChange={e => setFormular({ ...formular, motiv: e.target.value })} />
              </div>
            </div>
          </div>
          ...
          <div className="mt-8 pt-8 border-t border-slate-100 flex flex-col lg:flex-row gap-4 items-center">
            <div className="flex-1 w-full bg-slate-100/50 p-4 rounded-[30px] border border-slate-200">
              <div className="flex items-center justify-between mb-3 px-2">
                ...
                <label htmlFor="doc-upload" className="bg-slate-900 text-white px-5 py-2.5 rounded-xl font-black text-[10px] uppercase italic cursor-pointer hover:bg-amber-600 transition-colors shadow-sm active:scale-95">Adaugă Fișier +</label>
              </div>
              <div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto p-1">
                {formular.documente.map(doc => (
                  <div key={doc.id} className="relative flex items-center gap-2 w-auto max-w-[180px] h-10 pr-8 pl-2 bg-white border border-slate-200 rounded-xl shadow-sm">
                    ...
                    <button onClick={() => eliminaDocument(doc.id)} className="absolute right-1.5 w-5 h-5 bg-red-50 text-red-500 rounded-lg flex items-center justify-center text-[10px] font-black hover:bg-red-500 hover:text-white transition-all active:scale-90">✕</button>
                  </div>
                ))}
              </div>
            </div>
            <button onClick={salveazaInCloud} disabled={esteLimitat}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 pb-40">
          {programariAzi.length === 0 ? (
            ...
            programariAzi.map(p => {
              // ✅ Găsim numele din tabelele locale
              const spec = angajati.find(a => a.id === p.angajat_id);
              const serv = servicii.find(s => s.id === p.serviciu_id);
              return (
                <div key={p.id} className="relative bg-white p-5 rounded-[35px] shadow-sm border border-amber-200 ring-2 ring-amber-100 transition-all cursor-pointer hover:shadow-lg hover:scale-[1.02] active:scale-95"
                  onClick={() => setPopupProgramare(p)}>
                  <button onClick={e => { e.stopPropagation(); eliminaProgramare(p.id, e); }} className="absolute top-4 right-4 text-red-500 font-black text-[10px] z-10 hover:scale-125 transition-transform active:scale-90">✕</button>
      ...
      <Suspense fallback={null}>
        {popupProgramare && (
          <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[200] flex items-center justify-center p-4">
            <div ref={popupRef} className="bg-white w-full max-w-lg rounded-[50px] overflow-hidden shadow-2xl border border-slate-100 relative">
              <button onClick={() => setPopupProgramare(null)} className="absolute top-8 right-8 w-10 h-10 bg-slate-50 rounded-2xl flex items-center justify-center font-black text-slate-400 hover:bg-red-50 hover:text-red-500 transition-all z-10 active:scale-90">✕</button>
              <div className="h-32 bg-slate-900 relative">
                <div className="absolute -bottom-12 left-10 w-24 h-24 rounded-[30px] bg-white p-2 shadow-xl border border-slate-50">
                  <div className="w-full h-full rounded-[22px] bg-slate-50 overflow-hidden relative flex items-center justify-center">
                    {popupProgramare.poza ? <img src={popupProgramare.poza} className="w-full h-full object-cover" alt="client" /> : <Image src="/logo-chronos.png" alt="logo" fill sizes="80px" style={{ objectFit: 'contain', padding: '8px' }} />}
                  </div>
                </div>
              </div>
              <div className="pt-16 p-10">
                ...
                <div className="grid grid-cols-2 gap-4 mb-8">
                  ...
                  <div className="bg-slate-50 p-4 rounded-3xl border border-slate-100"><p className="text-[8px] font-black text-slate-400 uppercase italic mb-1">Serviciu</p><p className="font-black text-xs text-slate-700">{servicii.find(s => s.id === popupProgramare.serviciu_id)?.name || 'Procedură'}</p></div>
                </div>

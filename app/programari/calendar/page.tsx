...
function DayView({ selectedDate, programari, rawStaff, rawServices, serviceById, onEdit, adminWorkingHours, selectedExpert, selectedServiciu, onSelectServiciu, onAddNew }: {
  ...
  useEffect(() => {
    const target = whStart||`${new Date().getHours().toString().padStart(2,"00")}:00`;
            ...
            {slots.map((slot,i) => {
              ...
              return (
                <div key={slot} style={{
                  ...
                  background: isOff
                    ? (isClosed
                        ? "repeating-linear-gradient(135deg,rgba(239,68,68,0.07) 0px,rgba(239,68,68,0.07) 4px,rgba(255,245,245,1) 4px,rgba(255,245,245,1) 8px)"
                        : "repeating-linear-gradient(135deg,rgba(148,163,184,0.12) 0px,rgba(148,163,184,0.12) 4px,rgba(241,245,249,1) 4px,rgba(241,245,249,1) 8px)")
                    : "#fff",
                  borderTop: isHour?"1.5px solid #94a3b8":isHalf?"1px solid #cbd5e1":"1px solid #e2e8f0",
                }} />
              );
        ...
        {grid.map((day,idx)=>{
          ...
          return (
            <div key={idx} onClick={()=>onDayClick(day)}
              style={{minHeight:110,padding:"6px 6px 4px",display:"flex",flexDirection:"column",cursor:"pointer",borderBottom:"1px solid #e2e8f0",borderRight:(day.getDay()+6)%7<6?"1px solid #e2e8f0":"none",background:!isCurMo?"#f8fafc":isClosed?"repeating-linear-gradient(135deg,rgba(239,68,68,0.03) 0px,rgba(239,68,68,0.03) 5px,rgba(255,245,245,1) 5px,rgba(255,245,245,1) 10px)":isToday?"rgba(251,191,36,0.06)":"#fff",opacity:!isCurMo?0.38:1,transition:"background 0.15s"}}>
...
function CalendarContent() {
  ...
  const handleDelete = async()=>{
    if(!editForm)return;const ok=await showConfirm({title:"Ștergere",message:`Ștergi programarea lui ${editForm.nume}?`,confirmText:"Șterge",type:"danger"});
  ...
  // ─── FIX: Sanitizare nume fișier — elimină diacritice și caractere invalide ──
  const sanitizeFileName = (name: string): string => {
    return name
      .normalize("NFD")
      .replace(/[\u0300-\u036f]/g, "")      // elimină diacritice (ă→a, î→i, ș→s etc.)
      .replace(/[^a-zA-Z0-9._-]/g, "_");    // orice alt caracter invalid → _
  };
                ...
                <div style={{background:"#f8fafc",border:"1.5px solid #e2e8f0",borderRadius:12,padding:"8px 10px"}}>
                  ...
                  {(!editForm.documente||editForm.documente.length===0)&&!uploadingDoc&&(
                    <p style={{fontSize:9,color:"#cbd5e1",fontWeight:600,fontStyle:"italic",margin:0}}>
                      Niciun document — PDF, imagine, audio, video, orice fișier
                    </p>
                  )}
                  {uploadingDoc&&(
                  ...
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:8}}>
                    <div style={{flex:1}}>
                      ...
                      <input style={{width:"100%",background:"transparent",border:"none",fontSize:10,fontWeight:600,color:hasWA?"#334155":"#94a3b8",outline:"none",cursor:hasWA?"text":"not-allowed"}}
                        value={hasWA?customMsg:"Disponibil în planul ELITE sau TEAM"}

            ...
            {slots.map((slot,i) => {
              ...
              return (
                <div key={slot} style={{
                  ...
                  // ✅ În program = fundal simplu (gri foarte deschis, fără hașură).
                  // ✅ În afara programului = hașurat gri. Zi închisă = hașurat roșcat pe tot intervalul.
                  background: isClosed
              ...
              return (
                <button key={p.id}
                  ...
                  style={{
                    ...
                    // ✅ FIX: fără border shorthand mixt — fiecare latură separat
...
function WeekView({ selectedDate, programariByDate, rawStaff, serviceById, rawServices, onEdit, selectedExpert, selectedServiciu, adminWorkingHours, onSelectDate }: {
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",flex:1,overflow:"hidden"}}>
      ...
      {/* Grid săptămână — scroll doar pe body */}
          ...
          {weekDays.map((day,di)=>{
            ...
            return (
              <div key={di} style={{
                ...
                // ✅ Fond diferit pentru zi închisă vs zi normală
                background:isClosed
          ...
          return (
            <div key={idx} onClick={()=>onDayClick(day)}
              ...
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:4}}>
                ...
                <div style={{display:"flex",gap:3,alignItems:"center"}}>
                  {isClosed&&isCurMo&&<span style={{fontSize:8,fontWeight:800,color:"#fff",background:"#dc2626",padding:"2px 6px",borderRadius:5,letterSpacing:"0.02em"}}>🚫 ÎNCHIS</span>}
...
function DocumentsSection({ editForm, userId, setEditForm, qClient }: DocumentsSectionProps) {
  ...
  const handleDocUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of files) {
      try {
        // ✅ Sanitizăm numele fișierului (diacritice RO + caractere speciale)
        // Supabase Storage respinge fișierele cu diacritice — vezi sanitizeFileName()
    ...
    // ✅ Actualizăm editForm cu noile documente
    setEditForm(p => p ? { ...p, documente: docs } : null);
    // ✅ Salvăm imediat în appointments
    await supabase.from("appointments").update({ documente: docs }).eq("id", editForm.id);
    // ✅ Salvăm și în dosarul clientului din client_cases (după telefon)
    if (editForm.telefon) {
      ...
      if (caseData) {
        ...
        // Adăugăm doar fișierele noi (cele din docs care nu sunt deja în fisiere)
    ...
    // ✅ Reset input — altfel același fișier nu mai declanșează onChange a doua oară
    e.target.value = "";
...
function CalendarContent() {
  ...
  const handleUpdate = async()=>{
    ...
    const{error}=await supabase.from("appointments").update({
      ...
      // ✅ Salvăm și documentele actualizate
  ...
  const editExisting = useMemo(()=>{
    ...
    return programari.filter(p=>
      ...
      // ✅ Arătăm ore ocupate DOAR pentru același specialist
      // Dacă nu e ales specialist, nu blocăm nimic
  ...
  return (
    // ✅ Layout fix: header sus (flexShrink:0), body jos cu overflow
    <div style={{display:"flex",flexDirection:"column",height:"100vh",background:"#f8fafc",overflow:"hidden"}}>
          ...
          <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,0.72)",backdropFilter:"blur(6px)",zIndex:500,display:"flex",alignItems:"center",justifyContent:"center",padding:12}} onClick={closeModal}>
            <div ref={modalRef} onClick={e=>e.stopPropagation()} style={{background:"#fff",width:"100%",maxWidth:540,borderRadius:20,overflow:"hidden",boxShadow:"0 24px 60px rgba(0,0,0,0.25)",border:"1px solid #e2e8f0",position:"relative",display:"flex",flexDirection:"column",maxHeight:"96vh"}}>
              ...
              <div style={{padding:"12px 14px",display:"flex",flexDirection:"column",gap:7,overflowY:"auto",flex:1}}>
                ...
                {/* Rând 3: Specialist + Serviciu (filtrare dinamică) */}
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
                ...
                {/* Rând 5: Documente — upload + listă (componentă separată, hooks legale) */}
      ...
      {/* ── HEADER FIX (nu scrollează niciodată) ─────────────────────────── */}

...
// ─── Colors ───────────────────────────────────────────────────────────────────
const SC = [
  { border: "#3b82f6", chipBg: "#dbeafe", chipText: "#1d4ed8", chipBorder: "#93c5fd" },
  { border: "#10b981", chipBg: "#d1fae5", chipText: "#065f46", chipBorder: "#6ee7b7" },
  { border: "#8b5cf6", chipBg: "#ede9fe", chipText: "#5b21b6", chipBorder: "#c4b5fd" },
  { border: "#f59e0b", chipBg: "#fef3c7", chipText: "#92400e", chipBorder: "#fcd34d" },
  { border: "#f43f5e", chipBg: "#ffe4e6", chipText: "#9f1239", chipBorder: "#fda4af" },
  { border: "#06b6d4", chipBg: "#cffafe", chipText: "#155e75", chipBorder: "#67e8f9" },
  { border: "#6366f1", chipBg: "#e0e7ff", chipText: "#3730a3", chipBorder: "#a5b4fc" },
  { border: "#14b8a6", chipBg: "#ccfbf1", chipText: "#0f766e", chipBorder: "#5eead4" },
];
const SVC_C = [
...
// Grupează programările care încep la aceeași oră pentru layout side-by-side
function sameStartLayout(appts: Prog[]) {
  const groups: Record<string, Prog[]> = {};
  appts.forEach(p => {
    const key = p.ora || "00:00";
    if (!groups[key]) groups[key] = [];
    groups[key].push(p);
  });
  return appts.map(p => {
    const group = groups[p.ora || "00:00"] || [p];
    return { item: p, index: group.findIndex(x => x.id === p.id), count: group.length };
  });
}

// Limitează la maxVisible programări vizibile per oră; restul merg în hiddenByHour
function limitAppointmentsPerHour(appts: Prog[], maxVisible = 3) {
  const byHour: Record<string, Prog[]> = {};
  appts.forEach(p => {
    const h = String(Math.max(0, Math.min(23, Number(p.ora?.split(":")[0] || 0)))).padStart(2, "0");
    const key = `${h}:00`;
  ...
  const hiddenByHour: Record<string, Prog[]> = {};
  Object.entries(byHour).forEach(([hour, items]) => {
    const sorted = [...items].sort((a, b) => a.ora.localeCompare(b.ora));
    sorted.slice(0, maxVisible).forEach(p => visibleIds.add(p.id));
    if (sorted.length > maxVisible) hiddenByHour[hour] = sorted;
  });

  return { visibleIds, hiddenByHour };
}

// Calculează intervalul de slot-uri vizibile pentru săptămână (bazat pe orarul de lucru)
function slotRangeForDays(days: Date[], whByDay: Record<string, WorkingHour>) {
  const openDays = days
    .map(day => whByDay[TXT.dayLong[day.getDay()]])
    .filter(wh => wh && !wh.closed && wh.start && wh.end);

  const startMin = openDays.length ? Math.min(...openDays.map(wh => timeToMin(wh.start))) : 8 * 60;
  const endMin = openDays.length ? Math.max(...openDays.map(wh => timeToMin(wh.end === "00:00" ? "24:00" : wh.end))) : 18 * 60;
  const from = Math.max(0, startMin - 60);
  const to = Math.min(24 * 60, endMin + 60);
...
function AppointmentHoverCard({ prog, anchorRect, serviceById, rawStaff, staffColorIndex, onClose }: HoverCardProps) {
  ...
  useEffect(() => {
    ...
    const cardW = 284, cardH = cardRef.current?.offsetHeight || 260;
      ...
      <div ref={cardRef} style={{
        ...
        <div style={{ padding: "11px 15px", display: "flex", flexDirection: "column", gap: 7 }}>
          ...
          <div style={{ borderTop: "1px solid #f1f5f9", marginTop: 2, paddingTop: 7 }}>
            <span style={{ fontSize: 9, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.08em" }}>Click pe programare pentru a edita · Click oriunde pentru a închide</span>
          </div>
        </div>
      </div>
    </>
...
// ─── HourOverflowCard — popup cu toate programările dintr-o oră ───────────────
function HourOverflowCard({ hour, items, anchorRect, serviceById, rawStaff, staffMap, onEdit, onClose }: {
  hour: string; items: Prog[]; anchorRect: DOMRect;
  serviceById: Record<string, ServiceRow>; rawStaff: StaffRow[];
  staffMap: Record<string, number>;
  onEdit: (p: Prog) => void; onClose: () => void;
}) {
  const [pos, setPos] = useState({ top: 0, left: 0, ready: false });
  const cardRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    const vw = window.innerWidth, vh = window.innerHeight;
    const cardW = 340;
    const cardH = Math.min(cardRef.current?.offsetHeight || 400, vh - 24);
    let left = anchorRect.right + 12;
    if (left + cardW > vw - 12) left = anchorRect.left - cardW - 12;
    if (left < 12) left = Math.max(12, (vw - cardW) / 2);
    let top = anchorRect.top;
  ...
  return (
    <>
      <div style={{ position: "fixed", inset: 0, zIndex: 9998, background: "rgba(0,0,0,0.15)" }} onClick={onClose} />
      <div ref={cardRef} style={{
        ...
        {/* Header */}
        <div style={{ padding: "14px 16px 10px", background: "#0f172a", borderBottom: "3px solid #f59e0b" }}>
          <p style={{ fontSize: 9, fontWeight: 700, color: "#f59e0b", textTransform: "uppercase", letterSpacing: "0.1em", margin: 0 }}>Programări la aceeași oră</p>
          <h3 style={{ fontSize: 16, fontWeight: 700, color: "#fff", margin: "2px 0 0" }}>{hour} — {items.length} programări</h3>
        </div>
        {/* Lista programărilor */}
        <div style={{ padding: 10, display: "flex", flexDirection: "column", gap: 8 }}>
          {[...items].sort((a, b) => a.ora.localeCompare(b.ora)).map(p => {
            const svc = serviceById[p.serviciuId || ""];
            const staff = rawStaff.find(s => s.id === p.expertId);
            const ci = staffMap[p.expertId || ""] ?? 0;
            const color = SC[ci % SC.length];
            const endTime = svc?.duration ? addMinutesToTime(p.ora, svc.duration) : null;
            return (
              <button key={p.id} onClick={() => { onClose(); onEdit(p); }} style={{
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, marginBottom: 4 }}>
                  <span style={{ fontSize: 13, fontWeight: 800, color: "#0f172a", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{p.nume}</span>
                  <span style={{ fontSize: 11, fontWeight: 800, color: color.border, fontVariantNumeric: "tabular-nums", flexShrink: 0 }}>
                    {p.ora}{endTime ? ` → ${endTime}` : ""}
                  </span>
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  {svc && <span style={{ fontSize: 10, fontWeight: 700, color: color.chipText, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{svc.nume_serviciu}</span>}
                  {staff && <span style={{ fontSize: 10, color: "#64748b", flexShrink: 0 }}>· {staff.name}</span>}
                  {p.isOnline && <span style={{ fontSize: 10, flexShrink: 0 }}>🌐</span>}
                </div>
              </button>
            );
          })}
        ...
        <div style={{ padding: "0 10px 12px" }}>
          <button onClick={onClose} style={{ width: "100%", padding: "8px", background: "#f1f5f9", border: "none", borderRadius: 10, fontSize: 11, fontWeight: 700, color: "#64748b", cursor: "pointer" }}
            className="hover:bg-slate-200 transition-all">Închide</button>
        </div>
      </div>
    </>
  );
    ...
    <div style={{ flexShrink: 0, background: "#fff", borderBottom: "2px solid #e2e8f0" }}>
      <div style={{ display: "flex", alignItems: "stretch", minHeight: 64 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "0 12px", borderRight: "2px solid #e2e8f0", flexShrink: 0, minWidth: 190 }}>
          <button onClick={() => onSelectDate(addDays(weekStart, -7))} style={{ width: 28, height: 28, border: "1.5px solid #e2e8f0", borderRadius: 8, background: "#f8fafc", fontSize: 16, fontWeight: 700, color: "#334155", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>‹</button>
          <div style={{ flex: 1, textAlign: "center" }}>
            <p style={{ fontSize: 12, fontWeight: 700, color: "#1e293b", lineHeight: 1.3 }}>{monthLabel}</p>
            <button onClick={() => onSelectDate(today)} style={{ fontSize: 10, fontWeight: 700, color: "#d97706", background: "none", border: "none", cursor: "pointer", padding: 0 }}>→ Azi</button>
          </div>
          <button onClick={() => onSelectDate(addDays(weekStart, 7))} style={{ width: 28, height: 28, border: "1.5px solid #e2e8f0", borderRadius: 8, background: "#f8fafc", fontSize: 16, fontWeight: 700, color: "#334155", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>›</button>
        </div>
          ...
          {weekDays.map((day, i) => {
            ...
            return (
              <button key={i} onClick={() => onSelectDate(day)} style={{
                ...
                <span style={{ fontSize: 9, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", color: isSel ? "#f59e0b" : isToday ? "#d97706" : isClosed ? "#f87171" : "#64748b" }}>{TXT.dayShort[dow]}</span>
                <span style={{ fontSize: 18, fontWeight: 700, lineHeight: 1.2, color: isSel ? "#fff" : isToday ? "#d97706" : isClosed ? "#f87171" : "#1e293b" }}>{day.getDate()}</span>
                {total > 0 && (
...
function FilterBar({ rawStaff, rawServices, programari, selectedExpert, onSelectExpert, selectedServiciu, onSelectServiciu, selectedDate }: {
  ...
  const cntExp = useMemo(() => { const m: Record<string, number> = {}; programari.forEach(p => { if (p.data === dateKey && p.expertId) m[p.expertId] = (m[p.expertId] || 0) + 1; }); return m; }, [programari, dateKey]);
  const cntSvc = useMemo(() => { const m: Record<string, number> = {}; programari.forEach(p => { if (p.data === dateKey && p.serviciuId && (!selectedExpert || p.expertId === selectedExpert)) m[p.serviciuId] = (m[p.serviciuId] || 0) + 1; }); return m; }, [programari, dateKey, selectedExpert]);
  const visSvc = useMemo(() => { if (!selectedExpert) return rawServices; const st = rawStaff.find(s => s.id === selectedExpert); if (!st?.services?.length) return rawServices; return rawServices.filter(s => st.services.includes(s.id)); }, [selectedExpert, rawStaff, rawServices]);
    ...
    <div style={{ flexShrink: 0, background: "#fff", borderBottom: "2px solid #e2e8f0" }}>
      {rawStaff.length > 0 && (
        <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 14px", borderBottom: "1px solid #f1f5f9", overflowX: "auto", scrollbarWidth: "thin" }}>
          ...
          <button onClick={() => { onSelectExpert(""); onSelectServiciu(""); }} style={{ ...chipBase, background: !selectedExpert ? "#0f172a" : "#f8fafc", borderColor: !selectedExpert ? "#0f172a" : "#e2e8f0", color: !selectedExpert ? "#fff" : "#64748b" }}>Toți</button>
          {rawStaff.map((st, i) => {
      ...
      {rawServices.length > 0 && (
        <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 14px", overflowX: "auto", scrollbarWidth: "thin" }}>
          ...
          <button onClick={() => onSelectServiciu("")} style={{ ...chipBase, background: !selectedServiciu ? "#0f172a" : "#f8fafc", borderColor: !selectedServiciu ? "#0f172a" : "#e2e8f0", color: !selectedServiciu ? "#fff" : "#64748b" }}>Toate</button>
          {visSvc.map((svc, i) => {
  ...
  return (
    <div style={{ flexShrink: 0, background: "#f8fafc", borderTop: "1.5px solid #e2e8f0", display: "flex", alignItems: "center", gap: 10, padding: "6px 14px", overflowX: "auto", scrollbarWidth: "thin" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0, borderRight: "1.5px solid #e2e8f0", paddingRight: 10 }}>
        ...
        {totals.online > 0 && <div style={{ display: "flex", alignItems: "center", gap: 4 }}><span style={{ width: 8, height: 8, borderRadius: "50%", background: "#3b82f6", flexShrink: 0 }} /><span style={{ fontSize: 11, fontWeight: 700, color: "#2563eb" }}>{totals.online} online</span></div>}
      </div>
      {items.map(it => {
...
function DayView({ selectedDate, programari, rawStaff, rawServices, serviceById, onEdit, adminWorkingHours, selectedExpert, selectedServiciu, onSelectServiciu, onAddNew }: {
  ...
  const [hourOverflow, setHourOverflow] = useState<{ hour: string; items: Prog[]; rect: DOMRect } | null>(null);
  const hoverTimer = useRef<ReturnType<typeof setTimeout> | null>(null);

  // Slot-urile vizibile: dacă ziua e deschisă, arată doar intervalul de lucru ±1h; altfel toate
  const slots = useMemo(() => {
  ...
  // Filtrare programări pentru ziua curentă + filtrele active
  ...
  // Max 3 vizibile per oră, restul în popup
  const limited = useMemo(() => limitAppointmentsPerHour(dayAppts, 3), [dayAppts]);
  const visibleAppts = useMemo(
    () => dayAppts.filter(p => limited.visibleIds.has(p.id)).sort((a, b) => a.ora.localeCompare(b.ora)),
    [dayAppts, limited]
  );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", flex: 1, overflow: "hidden" }}>
      ...
      {/* Popup overflow ore */}
      {hourOverflow && (
        <HourOverflowCard hour={hourOverflow.hour} items={hourOverflow.items} anchorRect={hourOverflow.rect}
          serviceById={serviceById} rawStaff={rawStaff} staffMap={staffMap} onEdit={onEdit} onClose={() => setHourOverflow(null)} />
      )}
      ...
      <div ref={scrollRef} style={{ flex: 1, overflowY: "auto", overflowX: "hidden" }}>
        <div style={{ display: "flex", height: gridH, position: "relative" }}>
          ...
          <div style={{ flex: 1, position: "relative" }}>
            ...
            {slots.map((slot, i) => {
              ...
              return (
                <div key={slot} style={{
                  ...
                  // Hașuri: roșii pentru zi închisă, gri pentru ore în afara programului
                  background: isOff
                    ? isClosed
                      ? "repeating-linear-gradient(135deg,rgba(239,68,68,0.09) 0px,rgba(239,68,68,0.09) 4px,rgba(255,245,245,1) 4px,rgba(255,245,245,1) 10px)"
                      : "repeating-linear-gradient(135deg,rgba(148,163,184,0.13) 0px,rgba(148,163,184,0.13) 4px,rgba(241,245,249,1) 4px,rgba(241,245,249,1) 10px)"
                    : "#fff",
                  borderTop: isHour ? "1.5px solid #94a3b8" : isHalf ? "1px solid #cbd5e1" : "1px solid #e2e8f0",
                }} />
              );
            ...
            {/* Linii început/sfârșit program */}
            {!isClosed && whStart && whEnd && (() => {
              const s = ((timeToMin(whStart) - firstMin) / 15) * SLOT_H;
              const e = ((timeToMin(whEnd === "00:00" ? "24:00" : whEnd) - firstMin) / 15) * SLOT_H;
              return (
                <>
                  <div style={{ position: "absolute", left: 0, right: 0, top: s, height: 2, background: "#475569", zIndex: 10, pointerEvents: "none" }} />
                  <div style={{ position: "absolute", left: 0, right: 0, top: e, height: 2, background: "#475569", zIndex: 10, pointerEvents: "none" }} />
                  <div style={{ position: "absolute", right: 6, top: s + 4, fontSize: 8, fontWeight: 700, color: "#64748b", textTransform: "uppercase", letterSpacing: "0.06em", zIndex: 11, pointerEvents: "none", background: "#fff", padding: "1px 4px", borderRadius: 3 }}>Început program</div>
                  <div style={{ position: "absolute", right: 6, top: e + 4, fontSize: 8, fontWeight: 700, color: "#64748b", textTransform: "uppercase", letterSpacing: "0.06em", zIndex: 11, pointerEvents: "none", background: "#fff", padding: "1px 4px", borderRadius: 3 }}>Sfârșit program</div>
                </>
              );
            })()}
            ...
            {/* Butoane adăugare programare nouă pe slot gol */}
            {slots.map((slot, i) => {
            ...
            {/* Butoane "+N" pentru ore cu overflow */}
            {slots.filter(slot => slot.endsWith(":00") && limited.hiddenByHour[slot]).map(slot => {
              const topPx = ((timeToMin(slot) - firstMin) / 15) * SLOT_H + 4;
              const items = limited.hiddenByHour[slot];
              const extra = items.length - 3;
              return (
                <button key={`more-${slot}`}
                  onClick={e => { e.stopPropagation(); setHourOverflow({ hour: slot, items, rect: (e.currentTarget as HTMLElement).getBoundingClientRect() }); }}
                  style={{
                    position: "absolute", top: topPx, right: 10, zIndex: 30,
                    background: "#0f172a", color: "#f59e0b",
                    border: "1.5px solid #f59e0b", borderRadius: 999,
                    padding: "3px 10px", fontSize: 10, fontWeight: 800,
                    cursor: "pointer", boxShadow: "0 4px 12px rgba(15,23,42,0.2)",
                  }}
                  className="hover:bg-amber-500 hover:text-slate-900 hover:border-amber-500 transition-all">
                  +{extra} mai multe
                </button>
              );
            })}

            {/* Programările vizibile — side-by-side dacă sunt la aceeași oră */}
            {sameStartLayout(visibleAppts).map(({ item: p, index, count }) => {
              const svc = serviceById[p.serviciuId || ""];
              const endTime = svc?.duration ? addMinutesToTime(p.ora, svc.duration) : null;
              ...
              const ci = staffMap[p.expertId || ""] ?? 0;
              const color = SC[ci];
              // Dacă sunt mai multe la aceeași oră, le împarte orizontal
              const leftPct = count > 1 ? `calc(${(index / count) * 100}% + 8px)` : "8px";
              const rightPct = count > 1 ? `calc(${((count - index - 1) / count) * 100}% + 4px)` : "8px";
              return (
                <button key={p.id}
                  ...
                  style={{
                    position: "absolute", top: topPx + 2, height: heightPx,
                  ...
                  <p style={{ fontSize: 10, fontWeight: 700, color: color.border, lineHeight: 1.3, marginBottom: 1, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                    {p.ora}{endTime ? ` → ${endTime}` : ""}{p.isOnline ? " 🌐" : ""}
                  </p>
                  <p style={{ fontSize: 12, fontWeight: 700, color: "#1e293b", lineHeight: 1.3, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    {p.nume}
                  </p>
                  {svc && heightPx > 56 && (
                    <p style={{ fontSize: 10, color: "#64748b", lineHeight: 1.2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                      {svc.nume_serviciu}
                    </p>
                  )}
                </button>
...
function WeekView({ selectedDate, programariByDate, rawStaff, serviceById, rawServices, onEdit, selectedExpert, selectedServiciu, adminWorkingHours, onSelectDate }: {
  ...
  const today = new Date();
  const weekDays = useMemo(() => { const s = getWeekStart(selectedDate); return Array.from({ length: 7 }, (_, i) => addDays(s, i)); }, [selectedDate]);
  const whByDay = useMemo(() => { const m: Record<string, WorkingHour> = {}; adminWorkingHours.forEach(h => { m[h.day] = h; }); return m; }, [adminWorkingHours]);
  const staffMap = useMemo(() => { const m: Record<string, number> = {}; rawStaff.forEach((s, i) => { m[s.id] = i % SC.length; }); return m; }, [rawStaff]);
      ...
      <div style={{ flex: 1, overflowY: "auto", overflowX: "hidden" }}>
          ...
          {weekDays.map((day, i) => {
            ...
            return (
              <button key={i} onClick={() => onSelectDate(day)} style={{
                ...
                <span style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em", color: isClosed ? "#f87171" : isToday ? "#d97706" : "#64748b" }}>{TXT.dayShort[dow]}</span>
                <span style={{ fontSize: 22, fontWeight: 700, lineHeight: 1.2, color: isClosed ? "#f87171" : isToday ? "#d97706" : "#1e293b" }}>{day.getDate()}</span>
                {isClosed
                  ? <span style={{ fontSize: 8, fontWeight: 700, color: "#f87171", background: "#fee2e2", padding: "1px 6px", borderRadius: 99, marginTop: 2 }}>🚫 Închis</span>
                  : wh?.start ? <span style={{ fontSize: 9, color: "#94a3b8", fontWeight: 600, marginTop: 1 }}>{wh.start}–{wh.end}</span> : null}
                {appts.length > 0 && (
                  <span style={{ fontSize: 9, fontWeight: 700, padding: "2px 7px", borderRadius: 99, background: isToday ? "#f59e0b" : "#e2e8f0", color: isToday ? "#fff" : "#475569", marginTop: 3 }}>{appts.length} prog.</span>
                )}
              </button>
            );
          })}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", flex: 1, alignItems: "start" }}>
          {weekDays.map((day, di) => {
            ...
            const limited = limitAppointmentsPerHour(dayAppts, 3);
            const visible = dayAppts.filter(p => limited.visibleIds.has(p.id));
            ...
            return (
              <div key={di} style={{
                ...
                // Hașuri pentru zi închisă în week view
                background: isClosed
                  ? "repeating-linear-gradient(135deg,rgba(239,68,68,0.06) 0px,rgba(239,68,68,0.06) 4px,rgba(255,245,245,1) 4px,rgba(255,245,245,1) 10px)"
                ...
                {isClosed && (
                  <div style={{ textAlign: "center", padding: "12px 4px" }}>
                    ...
                    <span style={{ fontSize: 9, fontWeight: 700, color: "#f87171" }}>Zi închisă</span>
                  </div>
                )}
                {!isClosed && visible.length === 0 && (
                ...
                {/* Butoane overflow per oră */}
                {Object.entries(limited.hiddenByHour).map(([hour, items]) => (
                  <button key={`${key}-${hour}-more`}
                    onClick={e => { e.stopPropagation(); setHourOverflow({ hour, items, rect: (e.currentTarget as HTMLElement).getBoundingClientRect() }); }}
                    style={{
                      width: "100%", textAlign: "left", cursor: "pointer",
                      background: "#0f172a", color: "#f59e0b",
                      border: "1.5px solid #f59e0b", borderRadius: 7,
                      padding: "5px 8px", fontSize: 9, fontWeight: 800,
                    }}
                    className="hover:bg-amber-500 hover:text-slate-900 hover:border-amber-500 transition-all">
                    {hour} · +{Math.max(0, items.length - 3)} mai multe
                  </button>
                ))}
                ...
                {visible.map(p => {
                  ...
                  return (
                    <button key={p.id}
                      ...
                      style={{
                        width: "100%", textAlign: "left", cursor: "pointer",
                        background: color.chipBg, borderRadius: 8,
                        borderTop: `1px solid ${color.chipBorder}`,
                        borderRight: `1px solid ${color.chipBorder}`,
                        borderBottom: `1px solid ${color.chipBorder}`,
    ...
    <div style={{ display: "flex", flexDirection: "column", flex: 1, overflow: "auto" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", minWidth: 700, flex: 1 }}>
        {grid.map((day, idx) => {
          ...
          const wh = whByDay[TXT.dayLong[day.getDay()]]; const isClosed = !!wh?.closed;
          return (
            <div key={idx} onClick={() => onDayClick(day)}
              style={{
                ...
                background: !isCurMo ? "#f8fafc"
                  : isClosed ? "repeating-linear-gradient(135deg,rgba(239,68,68,0.04) 0px,rgba(239,68,68,0.04) 5px,rgba(255,245,245,1) 5px,rgba(255,245,245,1) 10px)"
                    : isToday ? "rgba(251,191,36,0.06)" : "#fff",
                opacity: !isCurMo ? 0.38 : 1, transition: "background 0.15s",
              }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
                <span style={{ fontSize: 12, fontWeight: 700, width: 24, height: 24, display: "flex", alignItems: "center", justifyContent: "center", borderRadius: 6, flexShrink: 0, background: isToday ? "#f59e0b" : isSel ? "#1e293b" : "transparent", color: isToday || isSel ? "#fff" : isClosed ? "#dc2626" : "#334155" }}>{day.getDate()}</span>
                <div style={{ display: "flex", gap: 3, alignItems: "center" }}>
                  {isClosed && isCurMo && <span style={{ fontSize: 7, fontWeight: 700, color: "#dc2626", background: "#fee2e2", padding: "1px 4px", borderRadius: 4 }}>Închis</span>}
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 2, flex: 1, overflow: "hidden" }}>
                ...
                {appts.length > 3 && <p style={{ fontSize: 9, fontWeight: 700, color: "#f59e0b", paddingLeft: 4 }}>+{appts.length - 3} mai multe</p>}
              </div>
            </div>
          );
        })}
          ...
          return (
            <div key={mo} onClick={() => onMonthClick(yr, mo)} style={{ background: "#fff", border: "1.5px solid #e2e8f0", borderRadius: 12, padding: "10px 12px", cursor: "pointer", transition: "all 0.15s", borderTop: isCurMo ? "3px solid #f59e0b" : isSel ? "3px solid #1e293b" : "1.5px solid #e2e8f0" }} className="hover:border-amber-400 hover:shadow-sm">
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                <span style={{ fontSize: 13, fontWeight: 700, color: isCurMo ? "#d97706" : "#334155" }}>{TXT.months[mo]}</span>
...
function CalendarContent() {
  ...
  const handleDocUpload = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of files) {
      try {
        // FIX: sanitizare nume fișier — elimină diacritice și caractere invalide pentru Supabase Storage
      ...
      } catch (err) { console.error("Upload eroare:", err); }
    }
    ...
    if (editForm.telefon) {
      ...
      const { data: caseData } = await supabase.from("client_cases").select("id,fisiere_atasate").eq("user_id", userId!).eq("phone_number", tel).single();
      if (caseData) {
        let fisiere: any[] = [];
        ...
        for (const doc of docs) {
          if (!existingUrls.has(doc.url)) {
            ...
            const tip = ["jpg", "jpeg", "png", "gif", "webp"].includes(ext) ? "imagine" : ["mp4", "mov", "avi", "webm"].includes(ext) ? "video" : ["mp3", "wav", "ogg", "m4a"].includes(ext) ? "audio" : "document";
            fisiere.push({ id: String(doc.id), nume: doc.name, url: doc.url, tip, created_at: new Date().toISOString() });
          }
        }
        await supabase.from("client_cases").update({ fisiere_atasate: JSON.stringify(fisiere) }).eq("id", caseData.id);
      }
    }
    setUploadingDoc(false);
    e.target.value = "";
    qClient.invalidateQueries({ queryKey: ["appointments", userId] });
  }, [editForm, userId, qClient]);
  ...
  const editExisting = useMemo(() => {
    if (!editForm) return [];
    return programari.filter(p => p.data === editForm.data && String(p.id) !== String(editForm.id) && !!editForm.expertId && p.expertId === editForm.expertId).map(p => ({ time: p.ora, duration: p.duration || 30 }));
              ...
              <div style={{ padding: "12px 14px", display: "flex", flexDirection: "column", gap: 7, overflowY: "auto", flex: 1 }}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 6 }}>
                  <div style={{ background: "#f8fafc", border: "1.5px solid #e2e8f0", borderRadius: 12, padding: "7px 10px" }}>
                    <p style={{ fontSize: 7, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", marginBottom: 2 }}>Nume</p>
                  ...
                  <button onClick={() => { setShowDatePicker(true); setShowTimePicker(false); }} style={{ background: "#0f172a", color: "#fff", border: "none", borderRadius: 12, padding: "7px 10px", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 1 }} className="hover:bg-slate-800 transition-all">
                    <span style={{ fontSize: 7, color: "#64748b", fontWeight: 700, textTransform: "uppercase" }}>Data</span>
                  ...
                  <button onClick={() => { setShowTimePicker(true); setShowDatePicker(false); }} style={{ background: "#0f172a", color: "#fff", border: "none", borderRadius: 12, padding: "7px 10px", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 1 }} className="hover:bg-slate-800 transition-all">
                    <span style={{ fontSize: 7, color: "#64748b", fontWeight: 700, textTransform: "uppercase" }}>Ora</span>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
                  <div style={{ background: "#0f172a", borderRadius: 12, padding: "7px 10px" }}>
                    <p style={{ fontSize: 7, fontWeight: 700, color: "#f59e0b", textTransform: "uppercase", marginBottom: 2 }}>Specialist</p>
                    <select style={{ width: "100%", background: "transparent", border: "none", fontSize: 11, fontWeight: 700, color: "#fff", outline: "none", cursor: "pointer" }}
                      ...
                      onChange={e => { const nid = e.target.value; const sp = rawStaff.find(s => s.id === nid); const ok = editForm.serviciuId && sp?.services?.includes(editForm.serviciuId); setEditForm(p => p ? { ...p, expertId: nid, serviciuId: ok ? p.serviciuId : "" } : null); }}>
                      <option value="" style={{ background: "#0f172a" }}>Alege...</option>
                      {(editForm.serviciuId ? rawStaff.filter(s => s.services?.includes(editForm.serviciuId!)) : rawStaff).map(o => <option key={o.id} value={o.id} style={{ background: "#0f172a" }}>{o.name}</option>)}
                    </select>
                  </div>
                  <div style={{ background: "#0f172a", borderRadius: 12, padding: "7px 10px" }}>
                    <p style={{ fontSize: 7, fontWeight: 700, color: "#f59e0b", textTransform: "uppercase", marginBottom: 2 }}>Serviciu</p>
                    <select style={{ width: "100%", background: "transparent", border: "none", fontSize: 11, fontWeight: 700, color: "#fff", outline: "none", cursor: "pointer" }}
                      value={editForm.serviciuId || ""}
                      onChange={e => { const nid = e.target.value; const sp = rawStaff.find(s => s.id === editForm.expertId); const ok = editForm.expertId && sp?.services?.includes(nid); setEditForm(p => p ? { ...p, serviciuId: nid, expertId: ok ? p.expertId : "" } : null); }}>
                      <option value="" style={{ background: "#0f172a" }}>Alege...</option>
                      {(editForm.expertId ? rawServices.filter(s => { const sp = rawStaff.find(x => x.id === editForm.expertId); return sp?.services?.includes(s.id); }) : rawServices).map(o => <option key={o.id} value={o.id} style={{ background: "#0f172a" }}>{o.nume_serviciu}</option>)}
                    </select>
                  </div>
                </div>
                <div style={{ background: "#f8fafc", border: "1.5px solid #e2e8f0", borderRadius: 12, padding: "7px 10px" }}>
                  <p style={{ fontSize: 7, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", marginBottom: 2 }}>Notițe</p>
                ...
                <div style={{ background: "#f8fafc", border: "1.5px solid #e2e8f0", borderRadius: 12, padding: "8px 10px" }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                    <p style={{ fontSize: 7, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", margin: 0 }}>
                      📎 Documente {editForm.documente?.length > 0 ? `(${editForm.documente.length})` : ""}
                    </p>
                    <label style={{ background: uploadingDoc ? "#64748b" : "#0f172a", color: "#fff", borderRadius: 7, padding: "3px 10px", fontSize: 9, fontWeight: 700, cursor: uploadingDoc ? "not-allowed" : "pointer", display: "flex", alignItems: "center", gap: 4, opacity: uploadingDoc ? 0.7 : 1 }}>
                      {uploadingDoc ? "⏳ Se încarcă..." : "+ Adaugă"}
                      <input type="file" multiple accept="*/*" style={{ display: "none" }} disabled={uploadingDoc} onChange={handleDocUpload} />
                    </label>
                  </div>
                  {(!editForm.documente || editForm.documente.length === 0) && !uploadingDoc && (
                    <p style={{ fontSize: 9, color: "#cbd5e1", fontWeight: 600, fontStyle: "italic", margin: 0 }}>Niciun document — PDF, imagine, audio, video, orice fișier</p>
                  )}
                  {uploadingDoc && (
                    <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "4px 0" }}>
                      ...
                      <span style={{ fontSize: 9, color: "#f59e0b", fontWeight: 700 }}>Se încarcă fișierele...</span>
                    </div>
                  )}
                  <div style={{ display: "flex", flexDirection: "column", gap: 4, maxHeight: 110, overflowY: "auto" }}>
                    {(editForm.documente || []).map((doc, i) => {
                      const ext = (doc.name || "").split(".").pop()?.toLowerCase() || "";
                      const icon = ["jpg", "jpeg", "png", "gif", "webp"].includes(ext) ? "🖼️" : ext === "pdf" ? "📄" : ["mp3", "wav", "ogg", "m4a"].includes(ext) ? "🎵" : ["mp4", "mov", "avi", "webm"].includes(ext) ? "🎬" : "📎";
                      return (
                        <div key={doc.id} style={{ display: "flex", alignItems: "center", gap: 6, background: "#fff", border: "1.5px solid #e2e8f0", borderRadius: 8, padding: "5px 8px" }}>
                          ...
                          <a href={doc.url} target="_blank" rel="noopener noreferrer" style={{ fontSize: 10, fontWeight: 700, color: "#334155", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", textDecoration: "none" }}>{doc.name}</a>
                          <button onClick={async () => { const newDocs = (editForm.documente || []).filter((_, j) => j !== i); setEditForm(p => p ? { ...p, documente: newDocs } : null); await supabase.from("appointments").update({ documente: newDocs }).eq("id", editForm.id); }} style={{ width: 18, height: 18, background: "#fee2e2", border: "none", borderRadius: 5, cursor: "pointer", fontSize: 9, fontWeight: 700, color: "#ef4444", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>✕</button>
                        </div>
                      );
                    })}
                  </div>
                ...
                <div style={{ border: "1.5px solid", borderColor: hasWA ? "#bbf7d0" : "#e2e8f0", borderRadius: 12, padding: "8px 10px", background: hasWA ? "#f0fdf4" : "#f8fafc", opacity: hasWA ? 1 : 0.6 }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                    <div style={{ flex: 1 }}>
                      <p style={{ fontSize: 7, fontWeight: 700, textTransform: "uppercase", color: hasWA ? "#15803d" : "#94a3b8", marginBottom: 3 }}>💬 WhatsApp</p>
                      <input style={{ width: "100%", background: "transparent", border: "none", fontSize: 10, fontWeight: 600, color: hasWA ? "#334155" : "#94a3b8", outline: "none", cursor: hasWA ? "text" : "not-allowed" }} value={hasWA ? customMsg : "Disponibil în planul ELITE sau TEAM"} onChange={e => { if (hasWA) setCustomMsg(e.target.value); }} readOnly={!hasWA} />
                    </div>
                    {hasWA && (
                      <button onClick={() => { const c = editForm.telefon?.replace(/\D/g, ""); window.open(`https://wa.me/${c?.startsWith("0") ? "4" + c : c}?text=${encodeURIComponent(customMsg)}`, "_blank"); }} style={{ background: "#16a34a", color: "#fff", border: "none", borderRadius: 8, padding: "6px 10px", fontSize: 10, fontWeight: 700, cursor: "pointer", flexShrink: 0, whiteSpace: "nowrap" }} className="hover:bg-green-700 transition-all">Trimite ↗</button>
                    )}
                  </div>
                </div>
                {/* Butoane */}
                <div style={{ display: "flex", gap: 6, paddingTop: 2, borderTop: "1.5px solid #f1f5f9" }}>
                  <button onClick={closeModal} style={{ flex: 1, padding: "8px", background: "#f1f5f9", border: "none", borderRadius: 12, fontSize: 11, fontWeight: 700, color: "#64748b", cursor: "pointer" }} className="hover:bg-slate-200 transition-all">Anulează</button>
                  <button onClick={handleUpdate} style={{ flex: 2, padding: "8px", background: "#0f172a", border: "none", borderRadius: 12, fontSize: 11, fontWeight: 700, color: "#fff", cursor: "pointer" }} className="hover:bg-amber-600 transition-all">✓ Salvează</button>
                  <button onClick={handleDelete} style={{ width: 36, padding: "8px", background: "#fff1f2", border: "1.5px solid #fecdd3", borderRadius: 12, fontSize: 14, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }} className="hover:bg-red-500 hover:text-white transition-all" title="Șterge">🗑️</button>
                </div>
              </div>
            </div>
          </div>
      ...
      {newForm && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.72)", backdropFilter: "blur(6px)", zIndex: 500, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }} onClick={() => setNewForm(null)}>
          <div style={{ background: "#fff", width: "100%", maxWidth: 480, borderRadius: 24, overflow: "hidden", boxShadow: "0 24px 60px rgba(0,0,0,0.22)", padding: 20, display: "flex", flexDirection: "column", gap: 10 }} onClick={e => e.stopPropagation()}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}><h2 style={{ fontSize: 16, fontWeight: 700, color: "#1e293b", margin: 0 }}>Programare nouă</h2><button onClick={() => setNewForm(null)} style={{ width: 32, height: 32, background: "#f1f5f9", border: "none", borderRadius: 10, cursor: "pointer", fontSize: 14, fontWeight: 700, color: "#64748b" }} className="hover:bg-red-500 hover:text-white transition-all">✕</button></div>
            <div style={{ background: "#fffbeb", border: "1.5px solid #fcd34d", borderRadius: 14, padding: "10px 14px" }}><p style={{ fontSize: 13, fontWeight: 700, color: "#92400e", margin: 0 }}>📅 {newForm.date} · ora {newForm.time}</p></div>
            <div style={{ background: "#f8fafc", border: "1.5px solid #e2e8f0", borderRadius: 14, padding: "10px 14px" }}><p style={{ fontSize: 8, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", marginBottom: 4 }}>Nume complet</p><input style={{ width: "100%", background: "transparent", border: "none", fontSize: 14, fontWeight: 700, color: "#1e293b", outline: "none" }} placeholder="Numele clientului..." value={newForm.nume} onChange={e => setNewForm(p => p ? { ...p, nume: e.target.value } : null)} /></div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>{[{ label: "Telefon", key: "telefon" }, { label: "Email", key: "email" }].map(f => (<div key={f.key} style={{ background: "#f8fafc", border: "1.5px solid #e2e8f0", borderRadius: 14, padding: "10px 14px" }}><p style={{ fontSize: 8, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", marginBottom: 4 }}>{f.label}</p><input style={{ width: "100%", background: "transparent", border: "none", fontSize: 12, fontWeight: 700, color: "#1e293b", outline: "none" }} value={(newForm as any)[f.key]} onChange={e => setNewForm(p => p ? { ...p, [f.key]: e.target.value } : null)} /></div>))}</div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>{[{ label: "Specialist", key: "expertId", opts: rawStaff, nameKey: "name" }, { label: "Serviciu", key: "serviciuId", opts: rawServices, nameKey: "nume_serviciu" }].map(f => (<div key={f.key} style={{ background: "#0f172a", borderRadius: 14, padding: "10px 14px" }}><p style={{ fontSize: 8, fontWeight: 700, color: "#f59e0b", textTransform: "uppercase", marginBottom: 4 }}>{f.label}</p><select style={{ width: "100%", background: "transparent", border: "none", fontSize: 11, fontWeight: 700, color: "#fff", outline: "none", cursor: "pointer" }} value={(newForm as any)[f.key]} onChange={e => setNewForm(p => p ? { ...p, [f.key]: e.target.value } : null)}><option value="" style={{ background: "#0f172a" }}>Alege...</option>{f.opts.map((o: any) => <option key={o.id} value={o.id} style={{ background: "#0f172a" }}>{(o as any)[f.nameKey]}</option>)}</select></div>))}</div>
            <div style={{ background: "#f8fafc", border: "1.5px solid #e2e8f0", borderRadius: 14, padding: "10px 14px" }}><p style={{ fontSize: 8, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", marginBottom: 4 }}>Notițe</p><textarea style={{ width: "100%", background: "transparent", border: "none", fontSize: 11, fontWeight: 700, color: "#334155", outline: "none", resize: "none" }} rows={2} value={newForm.motiv} onChange={e => setNewForm(p => p ? { ...p, motiv: e.target.value } : null)} /></div>

...
export default function Calendar({ programari }: CalendarProps) {
  ...
  const programariByDate = useMemo(() => {
    ...
    for (const key of Object.keys(map)) {
      map[key].sort((a, b) => compareTime(a.ora, b.ora));
    }
    return map;
  }, [programari]);
  ...
  return (
    <div className="w-full max-w-5xl bg-white rounded-2xl shadow-lg border border-amber-300 p-4 mt-8">
      {/* Header */}
          ...
          {monthGrid.map((day, idx) => {
            ...
            return (
              <button
                ...
                className={`min-h-[70px] rounded-lg border text-left px-2 py-1 flex flex-col ${
                  isSelected
      ...
      {viewMode === "week" && (
        <div className="overflow-x-auto">
          <div className="grid grid-cols-8 text-xs border-t border-l border-amber-200">
            <div className="bg-amber-50 border-b border-r border-amber-200 p-1 text-amber-800 font-semibold">
              Oră
            </div>
            {weekDays.map((d, i) => {
            ...
            {hours.map((h) => (
              <>
                <div
                  key={`h-${h}`}
                  className="border-b border-r border-amber-200 p-1 text-amber-800 bg-white text-xs"
                >
                  {String(h).padStart(2, "0")}:00
                </div>
                {weekDays.map((d, i) => {
                  const key = formatDateKey(d);
                  const list = (programariByDate[key] || []).filter(
                    (p) => parseInt(p.ora.slice(0, 2)) === h
                  );
                  return (
                    <div
                      key={`${key}-${h}-${i}`}
                    ...
                    >
                      {list.map((p) => (
                        <div
                          key={p.id}
                          className="mb-1 text-[10px] bg-amber-50 border border-amber-300 rounded px-1 text-amber-900 truncate"
                        >
                          {p.ora} • {p.nume}
                        </div>
                      ))}
        ...
        <div>
          ...
          <div className="grid grid-cols-[60px,1fr] text-xs border-t border-l border-amber-200">
            {hours.map((h) => {
              ...
              const list = (programariByDate[selectedKey] || []).filter(
                (p) => parseInt(p.ora.slice(0, 2)) === h
              );
              return (
                <>
                  <div
                    key={`hlabel-${h}`}
                    className="border-b border-r border-amber-200 p-1 text-amber-800 bg-amber-50"
                  >
                    {hourLabel}
                  </div>
                  <div
                    key={`hcell-${h}`}
                    className="border-b border-r border-amber-200 p-1 bg-white"
                  >
                    {list.map((p) => (
                      <div
                        key={p.id}
                        className="mb-1 text-[11px] bg-amber-50 border border-amber-300 rounded px-1 text-amber-900"
                      >
                        {p.ora} • {p.nume}
                      </div>
                    ))}
          ...
          {/* Lista programărilor din ziua selectată */}
          <div className="mt-4">

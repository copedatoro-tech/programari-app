...
import DosarClientComplet from "@/components/DosarClientComplet"; // ajustează calea dacă e diferită
...
export default function BazaDateClienti() {
  ...
  // --- Handler-e pentru DosarClientComplet ---
  ...
  const filtrati = useMemo(() => {
    ...
    return dosare.filter(
      (d) =>
        ...
        (d.phone_number || "").replace(/\D/g, "").includes(t.replace(/\D/g, "")) ||
        (d.phone_number || "").includes(t) ||
        (d.client_email || "").toLowerCase().includes(t)
    );
  }, [dosare, cautare]);
  ...
  return (
    <main className="min-h-screen bg-slate-50/50 p-4 md:p-12 font-sans text-slate-900">
      <div className="max-w-7xl mx-auto">
        ...
        {/* POP-UP DOSAR CLIENT - componenta nouă */}
